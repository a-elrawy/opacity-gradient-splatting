...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    if len(views) > 0:
        print(f"Average rendering time: {total_time / len(views):.4f} seconds")
        print(f"FPS: {len(views) / total_time:.2f}")
...
def render_video(source_path, model_path, iteration, views, gaussians, pipeline, background, fps=30):
    ...
    if len(render_poses) > 0:
        print(f"Average rendering time for video: {total_time / len(render_poses):.4f} seconds")
        print(f"Video FPS: {len(render_poses) / total_time:.2f}")
