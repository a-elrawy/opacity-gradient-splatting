            ...
            if  iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.densify_and_prune(opt.densify_grad_threshold, opt.prune_threshold, scene.cameras_extent, size_threshold, iteration,
                                              opt.use_error_densification, opt.error_densify_threshold, opt.max_gaussians)
                    gaussians.reset_densification_error()
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        # Save total points to a file
        with open(os.path.join(args.model_path, "total_points.txt"), "a") as f:
            f.write(f"{iteration} {scene.gaussians.get_xyz.shape[0]}\n")
