...
class GaussianModel:
    ...
    def __init__(self, args):
        ...
        self._densification_error_accum = torch.empty(0)
    ...
    def capture(self):
        return (
            ...
            self._densification_error_accum,
            self.denom,
    ...
    def restore(self, model_args, training_args):
        (self.active_sh_degree,
         ...
         self.max_radii2D,
         xyz_gradient_accum,
         densification_error_accum,
         denom,
         opt_dict,
         self.spatial_lr_scale) = model_args
        self.training_setup(training_args)
        self.xyz_gradient_accum = xyz_gradient_accum
        self._densification_error_accum = densification_error_accum
        self.denom = denom
    ...
    def training_setup(self, training_args):
        ...
        self._densification_error_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def densify_and_split(self, densification_metric, threshold, scene_extent, iteration, N=2):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        padded_metric = torch.zeros((n_init_points), device="cuda")
        padded_metric[:densification_metric.shape[0]] = densification_metric.squeeze()
        selected_pts_mask = torch.where(padded_metric >= threshold, True, False)
        ...
        prune_filter = torch.cat(
            (torch.zeros(n_init_points, device="cuda"), torch.ones(N * selected_pts_mask.sum(), device="cuda")))
        self.prune_points(prune_filter.to(torch.bool), iteration)

    def densify_and_clone(self, densification_metric, threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.where(densification_metric.squeeze() >= threshold, True, False)
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling,
                                                        dim=1).values <= self.percent_dense * scene_extent)
    ...
    def densify_and_prune(self, densify_grad_threshold, min_opacity, extent, max_screen_size, iteration,
                          use_error_densification, error_densify_threshold, max_gaussians):

        if use_error_densification:
            metric = self._densification_error_accum
            threshold = error_densify_threshold
        else:
            grads = self.xyz_gradient_accum / self.denom
            grads[grads.isnan()] = 0.0
            metric = torch.norm(grads, dim=-1, keepdim=True)
            threshold = densify_grad_threshold

        self.densify_and_clone(metric, threshold, extent)
    ...
    def update_densification_error(self, grads):
        self._densification_error_accum = torch.max(self._densification_error_accum, grads)

    def reset_densification_error(self):
        self._densification_error_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")